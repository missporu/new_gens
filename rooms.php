    ...
    default:
        ...
        <div class="row">
        	...
        	if ($set['ban']==1) {
                ...
                $rec =$sql->getOne("SELECT count(id) from chat WHERE tip=?i AND status=?i",$tip,1);
                $sum=10;
                $page = $ref[2];
                $get="tip=".$tip.":page";
                $posts = $rec;
		        ...
		        $data   = mysql_query("SELECT * FROM `chat` WHERE `tip`='" . $tip . "' AND `status`='1' ORDER BY `id` DESC LIMIT $start,$sum");
		        while ($rooms = mysql_fetch_assoc($data)) {
                    ...
                    if ($set['prava']>1) { ?>
		            	...
		            	<?php if ($set['prava']>1) {
                            ...
                            echo "<a class='media-right' href='?tip=".$tip."&ban=text&id_text=".$rooms['id']."'>[бан]</i></a></div>";
                        } ?>
        ...
        if (isset($_POST['text'])) {
            ...
            mysql_query("INSERT INTO `chat` SET `id_user` = '" . $user_id . "', `text` = '" . $text . "', `time` = '" . $timer . "', `date` = '" . $dater . "', `tip` = '" . $tip . "', `status`='1'");
