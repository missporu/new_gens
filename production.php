    ...
    $wah_up=$wah_count+1;
        ...
        if($gld==0){$sum="бесплатно";}else{$sum="за <img src='/images/icons/gold.png' alt='Gold'/> ".number_format_short($gld)."";}
    ...
    }else{echo"У вас недостаточно <img src='/images/icons/gold.png' alt='Gold'/>"; }
    }
    if($ref[1]=='build'){
    if($set['baks']>=$bax){
    $aki=$sql->getRow("SELECT * FROM production WHERE id=?i AND user_id=?i AND typ=?i",$ref[2],$user_id,1);
    if(!$aki or $aki[data]!=0){header('Location: ?mine');}else{
        if($ref[3]!='ok'){
        echo"<div class='col-md-12 text-center'><hr>
        <div class='main'><div class='block_zero center'><span style='color: #9cc;'>
        Участок $aki[num]
        ...
        <div class='mini-line'></div>
            ...
            case 4:$nam='фундамент';break;
            }
    if($aki[status]==4){$stt=0;}else{$stt=$aki[status];}
    ...
    }else{echo"У вас недостаточно <img src='//gmisspo.ru//images/icons/baks.png' alt='Baks'/>"; }
    }
    if($ref[1]=='log'){
    ...
    if($ref[1]=='work'){
    $aki=$sql->getRow("SELECT * FROM production WHERE id=?i AND user_id=?i AND typ=?i",$ref[2],$user_id,1);
    if(!$aki or $aki[data]!=0 or $aki[status]!=3 or $aki[hp]==0){header('Location: ?mine');}else{
    $tmr=$_POST['timer'];
    if($tmr<1 or $tmr>6 or !$tmr ){$tmr=1;}
    switch($tmr){
    case 1:$tm=600;$tm2=10;break;
    case 2:$tm=1200;$tm2=20;break;
    case 3:$tm=1800;$tm2=30;break;
    case 4:$tm=2400;$tm2=40;break;
    case 5:$tm=3600;$tm2=60;break;
    case 6:$tm=5400;$tm2=90;break;
    }
    if($aki[timer]>=$tm2){
    $gl=round((($aki[gold]/30)/90)*$tm2);

    if($aki[hp_a]>=29){$gl=($aki[gold]-$aki[gold_a]);}
